...
public:
    ...
    static void SetFirmwareVersionFile(SceneSetApp& app, const std::string& filePath) {
        app.m_firmwareVersionFileOverride = filePath;
    }

    static void SetLastFirmwareVersionMarker(SceneSetApp& app, const std::string& markerPath) {
...
static void SetLastTerminationNature(SceneSetApp& app, int value) {
    ...
    static std::string GetLastTelemetryPayload(const SceneSetApp& app) {
        return app.m_lastTelemetryPayload;
    }

    static void ClearLastTelemetry(SceneSetApp& app) {
        app.m_lastTelemetryMarker.clear();
        app.m_lastTelemetryPayload.clear();
    }
...
protected:
    ...
    void TearDown() override {
        ...
        unsetenv("SCENESET_OVERRIDE_CONFIG_FILE");
    }
};
...
// --- isFirmwareChanged / firmware version tests ---

// Test isFirmwareChanged returns true when no previous firmware version is stored
TEST_F(SceneSetTest, IsFirmwareChangedReturnsTrueWhenNoPreviousFirmwareStored) {
    SceneSetApp app;
    ...
    EXPECT_TRUE(app.isFirmwareChanged());

    std::filesystem::remove(versionFile, ec);
}

// Test isFirmwareChanged returns false when current firmware matches the stored version
TEST_F(SceneSetTest, IsFirmwareChangedReturnsFalseWhenFirmwareUnchanged) {
    SceneSetApp app;
    const auto versionFile = MakeUniqueTempPath("sceneset_version_txt");
    const auto markerPath = MakeUniqueTempPath("sceneset_last_fw_version");
    ...
    {
        std::ofstream f(versionFile);
        f << "imagename:TEST_FW_2.0" << std::endl;
    }
    {
        std::ofstream m(markerPath);
        m << "TEST_FW_2.0" << std::endl;
    }

    SceneSetAppTestPeer::SetFirmwareVersionFile(app, versionFile.string());
    SceneSetAppTestPeer::SetLastFirmwareVersionMarker(app, markerPath.string());
    EXPECT_FALSE(app.isFirmwareChanged());
...
// Test isFirmwareChanged returns true when current firmware differs from stored version
TEST_F(SceneSetTest, IsFirmwareChangedReturnsTrueWhenFirmwareDiffers) {
    SceneSetApp app;
    const auto versionFile = MakeUniqueTempPath("sceneset_version_txt");
    const auto markerPath = MakeUniqueTempPath("sceneset_last_fw_version");
    ...
    {
        std::ofstream f(versionFile);
        f << "imagename:TEST_FW_3.0" << std::endl;
    }
    {
        std::ofstream m(markerPath);
        m << "TEST_FW_2.0" << std::endl;
...
// Test isFirmwareChanged returns true when firmware version file cannot be read
TEST_F(SceneSetTest, IsFirmwareChangedReturnsTrueWhenVersionFileNotFound) {
    SceneSetApp app;
...
// Test readCurrentFirmwareVersion extracts imagename from version file
TEST_F(SceneSetTest, ReadCurrentFirmwareVersionExtractsImageName) {
    SceneSetApp app;
    const auto versionFile = MakeUniqueTempPath("sceneset_version_txt");

    {
        std::ofstream f(versionFile);
        f << "imagename:SCXI11BEI_MIDDLEWARE_DEV_develop_20260722111523" << std::endl;
    }

    SceneSetAppTestPeer::SetFirmwareVersionFile(app, versionFile.string());
    EXPECT_EQ(app.readCurrentFirmwareVersion(), "SCXI11BEI_MIDDLEWARE_DEV_develop_20260722111523");

    std::error_code ec;
    std::filesystem::remove(versionFile, ec);
}

// Test readCurrentFirmwareVersion returns empty when version file is missing
TEST_F(SceneSetTest, ReadCurrentFirmwareVersionReturnsEmptyWhenFileMissing) {
    SceneSetApp app;
    const auto versionFile = MakeUniqueTempPath("sceneset_version_txt_missing");
    ...
    SceneSetAppTestPeer::SetFirmwareVersionFile(app, versionFile.string());
    EXPECT_TRUE(app.readCurrentFirmwareVersion().empty());
}

// Test readCurrentFirmwareVersion returns empty when imagename line is absent
TEST_F(SceneSetTest, ReadCurrentFirmwareVersionReturnsEmptyWhenNoImageNameLine) {
    SceneSetApp app;
    const auto versionFile = MakeUniqueTempPath("sceneset_version_txt");

    {
        std::ofstream f(versionFile);
        f << "some_other_key:some_value" << std::endl;
        f << "another_key:another_value" << std::endl;
    }
...
// --- storeCurrentFirmwareVersion tests ---

// Test storeCurrentFirmwareVersion creates the marker file
TEST_F(SceneSetTest, StoreCurrentFirmwareVersionCreatesMarkerFile) {
    SceneSetApp app;
    const auto versionFile = MakeUniqueTempPath("sceneset_version_txt");
    const auto markerPath = MakeUniqueTempPath("sceneset_last_fw_version");
...
// Test storeCurrentFirmwareVersion causes isFirmwareChanged to return false on same firmware
TEST_F(SceneSetTest, StoreCurrentFirmwareVersionCausesIsFirmwareChangedToReturnFalse) {
    SceneSetApp app;
    const auto versionFile = MakeUniqueTempPath("sceneset_version_txt");
    const auto markerPath = MakeUniqueTempPath("sceneset_last_fw_version");
...
protected:
    ...
    void TearDown() override {
        ...
        SceneSetAppTestPeer::SetPendingActiveTelemetry(instance, false);
        SceneSetAppTestPeer::SetLaunchRequestTimestampMs(instance, 0);
        SceneSetAppTestPeer::SetSceneSetStartTimestampMs(instance, 0);
        SceneSetAppTestPeer::SetPreinstallStartTimestampMs(instance, 0);
        SceneSetAppTestPeer::SetPreinstallEndTimestampMs(instance, 0);
        SceneSetAppTestPeer::SetCumulativeRelaunchCount(instance, 0);
...
TEST_F(AppManagerEventHandlerTest, OnActivePublishesInitialLaunchTelemetryWithoutRelaunchContextFields) {
    ...
    SceneSetAppTestPeer::SetCumulativeRelaunchCount(instance, 0);
    SceneSetAppTestPeer::SetLastTerminationNature(instance, 0);
    SceneSetAppTestPeer::ClearLastTelemetry(instance);
...
TEST_F(AppManagerEventHandlerTest, UnloadedAbortIncrementsRelaunchCountAndSetsCrashNature) {
    SceneSetApp& instance = SceneSetApp::getInstance();
    const std::string& refId = SceneSetAppTestPeer::GetReferenceAppId(instance);
    if (refId.empty()) {
        GTEST_SKIP() << "Reference app ID is empty; skipping.";
    ...
    EXPECT_EQ(SceneSetAppTestPeer::GetCumulativeRelaunchCount(instance), 1u);
    EXPECT_EQ(SceneSetAppTestPeer::GetLastTerminationNature(instance), 1);
}

TEST_F(AppManagerEventHandlerTest, PendingRestartUnloadedIncrementsRelaunchCountAndSetsIntentionalKillNature) {
    SceneSetApp& instance = SceneSetApp::getInstance();
    const std::string& refId = SceneSetAppTestPeer::GetReferenceAppId(instance);
    if (refId.empty()) {
        GTEST_SKIP() << "Reference app ID is empty; skipping.";
    ...
    SceneSetAppTestPeer::SetAppLaunched(instance, true);
    SceneSetAppTestPeer::SetPendingRestart(instance, true);
    SceneSetAppTestPeer::SetCumulativeRelaunchCount(instance, 2);
    ...
    EXPECT_FALSE(SceneSetAppTestPeer::GetPendingRestart(instance));
    EXPECT_EQ(SceneSetAppTestPeer::GetCumulativeRelaunchCount(instance), 3u);
    EXPECT_EQ(SceneSetAppTestPeer::GetLastTerminationNature(instance), 2);
}
...
TEST_F(SceneSetTest, LoadSystemConfigReadsDefaultHomeAppWhenPresent) {
    SceneSetApp app;
    const auto configPath = MakeUniqueTempPath("sceneset_system_config_default_home_app");
    {
        std::ofstream configFile(configPath);
        configFile << "defaultHomeApp=org.rdk.EPG" << std::endl;
    }

    setenv("SCENESET_SYSTEM_CONFIG_FILE", configPath.c_str(), 1);

    std::unordered_map<std::string, std::string> values;
    ASSERT_TRUE(SceneSetAppTestPeer::LoadSystemConfig(app, values));
    ASSERT_TRUE(values.find("defaultHomeApp") != values.end());
    EXPECT_EQ(values["defaultHomeApp"], "org.rdk.EPG");

    std::error_code ec;
    std::filesystem::remove(configPath, ec);
}

TEST_F(SceneSetTest, LoadSystemConfigHandlesMissingOrEmptyDefaultHomeApp) {
    SceneSetApp app;

    const auto missingKeyConfigPath = MakeUniqueTempPath("sceneset_system_config_no_default_home_app");
    {
        std::ofstream configFile(missingKeyConfigPath);
        configFile << "preinstallLocation=/tmp/preinstall_from_config" << std::endl;
    }

    setenv("SCENESET_SYSTEM_CONFIG_FILE", missingKeyConfigPath.c_str(), 1);

    std::unordered_map<std::string, std::string> values;
    ASSERT_TRUE(SceneSetAppTestPeer::LoadSystemConfig(app, values));
    EXPECT_TRUE(values.find("defaultHomeApp") == values.end());

    const auto emptyKeyConfigPath = MakeUniqueTempPath("sceneset_system_config_empty_default_home_app");
    {
        std::ofstream configFile(emptyKeyConfigPath);
        configFile << "defaultHomeApp=" << std::endl;
    }

    setenv("SCENESET_SYSTEM_CONFIG_FILE", emptyKeyConfigPath.c_str(), 1);
    ASSERT_TRUE(SceneSetAppTestPeer::LoadSystemConfig(app, values));
    ASSERT_TRUE(values.find("defaultHomeApp") != values.end());
    EXPECT_TRUE(values["defaultHomeApp"].empty());

    std::error_code ec;
    std::filesystem::remove(missingKeyConfigPath, ec);
    std::filesystem::remove(emptyKeyConfigPath, ec);
}

TEST_F(SceneSetTest, LoadSystemConfigOverrideBehaviorMatchesBuildType) {
    SceneSetApp app;
    const auto baseConfigPath = MakeUniqueTempPath("sceneset_system_config_base");
    const auto overrideConfigPath = MakeUniqueTempPath("sceneset_system_config_override");
    {
        std::ofstream configFile(baseConfigPath);
        configFile << "preinstallLocation=/tmp/base_preinstall" << std::endl;
        configFile << "defaultHomeApp=org.rdk.BaseHome" << std::endl;
    }
    {
        std::ofstream configFile(overrideConfigPath);
        configFile << "defaultHomeApp=org.rdk.OverrideHome" << std::endl;
    }

    setenv("SCENESET_SYSTEM_CONFIG_FILE", baseConfigPath.c_str(), 1);
    setenv("SCENESET_OVERRIDE_CONFIG_FILE", overrideConfigPath.c_str(), 1);
...
#endif
    EXPECT_EQ(values["preinstallLocation"], "/tmp/base_preinstall");

    std::error_code ec;
    std::filesystem::remove(baseConfigPath, ec);
    std::filesystem::remove(overrideConfigPath, ec);
}
